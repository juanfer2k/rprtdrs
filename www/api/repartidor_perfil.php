...
function getUser($pdo) {
    ...
    // Fallback: token en body o query
    if (!$token) $token = isset($_POST['token']) ? trim($_POST['token']) : '';
    if (!$token) $token = isset($_GET['token'])  ? trim($_GET['token'])  : '';
...
switch ($action) {
    ...
    case 'get':
        $stmt = $pdo->prepare("
            SELECT r.id_repartidor, r.nombre_completo, r.telefono, r.email,
                   r.foto_url, r.estado, u.username
            FROM repartidores r
            JOIN usuarios u ON u.id = r.id_repartidor
            WHERE u.id = ?
            LIMIT 1
        ");
        ...
        if (!$perfil) {
            // Fallback: buscar repartidor por id directamente aunque no coincida el JOIN
            $stmt2 = $pdo->prepare("SELECT * FROM repartidores WHERE id_repartidor = ? LIMIT 1");
            $stmt2->execute(array($uid));
            $rep = $stmt2->fetch();
            if ($rep) {
                $rep['username'] = $user['username'];
                jsonOk(array('perfil' => $rep));
            }
            jsonErr('Perfil no encontrado', 404);
        }

        jsonOk(array('perfil' => $perfil));

    // ── Actualizar telefono / email ───────────────────────────────────────────
    case 'update_profile':
        ...
        // Intentar actualizar por id_repartidor = uid (IDs coinciden)
        $stmt = $pdo->prepare("UPDATE repartidores SET telefono = ?, email = ? WHERE id_repartidor = ?");
        $stmt->execute(array($telefono, $email, $uid));
    ...
    case 'change_password':
        ...
        $stmt->execute(array($uid));
        $row = $stmt->fetch();
        if (!$row || !password_verify($pwd_actual, $row['password_hash'])) {
            jsonErr('Contraseña actual incorrecta', 403);
        }

        $hash = password_hash($pwd_nueva, PASSWORD_DEFAULT);
        $pdo->prepare("UPDATE usuarios SET password_hash = ? WHERE id = ?")->execute(array($hash, $uid));
        jsonOk();

    // ── Subir foto ────────────────────────────────────────────────────────────
    case 'upload_photo':
        if (!isset($_FILES['foto']) || $_FILES['foto']['error'] !== UPLOAD_ERR_OK) {
            jsonErr('No se recibió foto válida');
        }

        $ext = strtolower(pathinfo($_FILES['foto']['name'], PATHINFO_EXTENSION));
        if (!in_array($ext, array('jpg','jpeg','png','webp','gif'))) {
            jsonErr('Formato no permitido');
        }
        ...
        if (!move_uploaded_file($_FILES['foto']['tmp_name'], $dest)) {
            jsonErr('Error al guardar la imagen', 500);
        }

        $fotoUrl = 'uploads/' . $fileName;

        // Borrar foto anterior
        $old = $pdo->prepare("SELECT foto_url FROM repartidores WHERE id_repartidor = ? LIMIT 1");
        $old->execute(array($uid));
        $oldRow = $old->fetch();
        if ($oldRow && $oldRow['foto_url']) {
            $oldPath = __DIR__ . '/../' . $oldRow['foto_url'];
            if (file_exists($oldPath)) @unlink($oldPath);
        }

        $pdo->prepare("UPDATE repartidores SET foto_url = ? WHERE id_repartidor = ?")->execute(array($fotoUrl, $uid));
