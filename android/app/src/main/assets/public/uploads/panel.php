...
<head>
    ...
    <!-- Leaflet CSS desde CDN (Más fiable) -->
...
<script>
    ...
    function renderTable(repartidores) {
        document.getElementById('driver-table').innerHTML = repartidores.map(r => {
            ...
            return `
                <tr class="driver-row" ${clickAction}>
                    <td><strong>${r.nombre_completo}</strong></td>
                    <td><span class="badge ${isAvailable ? 'bg-success' : 'bg-warning'}">${r.estado}</span></td>
                    <td><small>${r.ultima_actualizacion || 'Nunca'}</small></td>
                    <td><button class="btn btn-sm ${hasCoords ? 'btn-primary' : 'btn-outline-secondary disabled'}">Ver</button></td>
                </tr>
            `;
        }).join('');
    }
